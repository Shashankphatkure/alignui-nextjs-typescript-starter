...
const ORDER = {
  ...
  timeline: [
    {
      status: 'Shipment Order Received',
      time: '09-05-2026 18:12',
      by: 'Sunilkumar',
      done: true,
    },
    {
      status: 'Shipment Picked Up',
      time: '09-05-2026 22:24',
      by: 'Amarnath',
      done: true,
    },
    {
...
function InfoRow({ label, value }: InfoRowProps) {
  return (
    <div className='flex items-start justify-between gap-4 border-b border-stroke-soft-200 py-2.5 last:border-0'>
      <span className='min-w-[120px] shrink-0 text-paragraph-sm text-text-sub-600'>
        {label}
      </span>
      <span className='text-right text-paragraph-sm font-medium text-text-strong-950'>
        {value}
      </span>
    </div>
  );
...
function SectionCard({
  title,
  icon: Icon,
  iconColor = 'bg-bg-soft-200 text-text-sub-600',
  children,
}: SectionCardProps) {
  return (
...
export default function OrderDetailPage() {
  ...
  const statusColor = (STATUS_TO_BADGE_COLOR[ORDER.status] ??
    'gray') as BadgeColor;
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 gap-5 lg:grid-cols-3'>
        ...
        <div className='flex flex-col gap-5'>
          ...
          <div className='overflow-hidden rounded-2xl border border-stroke-soft-200 bg-bg-white-0 shadow-regular-xs'>
            <div className='flex items-center justify-between border-b border-stroke-soft-200 px-5 py-3.5'>
              ...
              <button className='text-text-sub-600 transition hover:text-text-strong-950'>
                <RiMoreLine size={16} />
              </button>
            </div>
                  ...
                  <div className='flex flex-col items-center'>
                    <div
                      className={cn(
                        ...
                        event.done &&
                          i === ORDER.timeline.filter((e) => e.done).length - 1
                          ? 'shadow-lg bg-primary-base ring-2 ring-primary-alpha-24'
                          : event.done
                            ? 'bg-success-base'
                            : 'border border-stroke-soft-200 bg-bg-soft-200',
                      )}
                    >
                      {event.done ? (
          ...
          >
            <div className='space-y-2 py-3'>
              {ORDER.barcodes.map((bc) => (
                <div
                  key={bc}
                  className='flex items-center justify-between gap-2 rounded-xl bg-bg-weak-50 px-3 py-2.5'
                >
                  <span className='truncate font-mono text-paragraph-xs text-text-strong-950'>
                    {bc}
                  </span>
                  <Tooltip.Root>
                    <Tooltip.Trigger asChild>
                      <button className='shrink-0 text-text-sub-600 transition hover:text-primary-base'>
          ...
          >
            <div className='space-y-3 py-3'>
              {ORDER.invoices.map((inv, i) => (
                <div
                  key={i}
                  className='space-y-1.5 rounded-xl bg-bg-weak-50 p-3'
                >
        ...
        <div className='flex flex-col gap-5 lg:col-span-2'>
          ...
          >
            <div className='py-1'>
              <div className='grid grid-cols-2 gap-x-8'>
                ...
                <InfoRow
                  label='Delivery Type'
                  value={ORDER.tariff.deliveryType}
                />
                <InfoRow label='COD' value={ORDER.tariff.cod} />
                <InfoRow label='Total Quantity' value={ORDER.tariff.quantity} />
                <InfoRow
                  label='Actual Weight'
                  value={`${ORDER.tariff.actualWeight} kg`}
                />
                <InfoRow
                  label='Chargeable Wt'
                  value={`${ORDER.tariff.chargeableWeight} kg`}
                />
              </div>
            </div>
          </SectionCard>
                ...
                <thead>
                  <tr className='border-b border-stroke-soft-200'>
                    {[
                      ...
                      <th
                        key={h}
                        className='pb-2 pr-4 text-left text-paragraph-xs font-semibold text-text-sub-600'
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className='divide-y divide-stroke-soft-200'>
                  {ORDER.dimensions.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td className='py-2.5 pr-4 font-medium text-text-strong-950'>
                        {d.length}
                      </td>
                      <td className='py-2.5 pr-4 font-medium text-text-strong-950'>
                        {d.breadth}
          ...
          {ORDER.cold && (
            ...
            >
              <div className='py-1'>
                ...
                <InfoRow
                  label='Temperature'
                  value={ORDER.coldChain.temperature}
                />
                <InfoRow
                  label='Logger No'
                  value={
                    <span
                      className='max-w-[240px] truncate text-right font-mono text-paragraph-xs'
                      title={ORDER.coldChain.loggerNo}
                    >
          ...
          <SectionCard
            title='Order Images'
            icon={RiImageLine}
            iconColor='bg-highlighted-lighter text-highlighted-base'
          >
            <div className='py-4'>
              <div className='flex items-center justify-center rounded-xl border-2 border-dashed border-stroke-soft-200 py-8 text-center'>
                <div className='space-y-1.5'>
                  <RiImageLine
                    size={28}
                    className='mx-auto text-text-disabled-300'
                  />
      ...
      <Drawer.Root
        ...
        onOpenChange={(open) => !open && setUpdateDrawerOpen(false)}
      >
        <Drawer.Content>
          <Drawer.Header>
            ...
            <div className='space-y-6 px-5 py-6'>
              ...
              <div className='space-y-1.5'>
                ...
                <Select.Root size='small' onValueChange={setNewStatus}>
                  ...
                  <Select.Content>
                    <Select.Item value='Shipment Order Received'>
                      Shipment Order Received
                    </Select.Item>
                    <Select.Item value='Shipment Picked Up'>
                      Shipment Picked Up
                    </Select.Item>
                    <Select.Item value='Shipment Arrived At Hub'>
                      Shipment Arrived At Hub
                    </Select.Item>
                    <Select.Item value='Shipment In Transit'>
                      Shipment In Transit
                    </Select.Item>
                    <Select.Item value='Out For Delivery'>
                      Out For Delivery
                    </Select.Item>
